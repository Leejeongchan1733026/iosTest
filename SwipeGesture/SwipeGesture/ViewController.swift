...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //각 배열의 첫번째 이미지를 뷰에 할당
        imgViewUp.image = imgUp[0]
        imgViewDown.image = imgDown[0]
        imgViewLeft.image = imgLeft[0]
        imgViewRight.image = imgRight[0]
        
        let swipeUp = UISwipeGestureRecognizer(target: self, action: #selector(ViewController.respondToSwipeGesture(_:))) //스와이프를 실행했을 때 실행할 메서드를 의미하는 상수 선언
        swipeUp.direction = UISwipeGestureRecognizer.Direction.up //위쪽 방향으로 스와이프 했을 때 속성
        self.view.addGestureRecognizer(swipeUp) //제스쳐 등록
        ...
        swipeDownMulti.numberOfTouchesRequired = numOfTouchs //두개를 눌렀을 때
